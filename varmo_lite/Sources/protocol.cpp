...
#endif
/*
protocol_setting define_protocol_setting(char* serial){
	protocol_setting setting;
	setting.PROTOCOL= (char*) "ExmEisla";
	setting.SERIAL_NUMBER = serial;
	setting.DELIMITATOR = ':';
	setting.END = (char*) "\r\n";
	return setting;
}*/
...
/*
void serial_send_block(protocol_setting setting, char* data1 = (char*)"", char* data2 = (char*)"")
{
...
void serial_send_block(char* data1 = (char*)"", char* data2 = (char*)"")
{
	int length = sizeof(protocol_setting) + sizeof(data1) + sizeof(data2);

	binaryRepr length_block = format_length(length);

	serial_send_string((char*)protocol_setting.PROTOCOL);
	serial_send_char(length_block.toInt.int1);
	serial_send_char(length_block.toInt.int0);
	serial_send_string((char*)protocol_setting.SERIAL_NUMBER);
	serial_send_string(data1);
	serial_send_char((char)protocol_setting.DELIMITATOR);
	serial_send_string(data2);
	serial_send_string((char*)protocol_setting.END);
}
